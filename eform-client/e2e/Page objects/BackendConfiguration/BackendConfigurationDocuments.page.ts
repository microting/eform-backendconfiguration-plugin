...
export class BackendConfigurationFilesPage extends Page {
  ...
  public async backendConfigurationPnDocumentsButton() {
    const ele = await $('#backend-configuration-pn-documents');
    await ele.waitForDisplayed({timeout: 40000});
    await ele.waitForClickable({timeout: 40000});
    return ele;
  }

  public async createNewDocumentBtn() {
    const ele = await $('#createNewDocumentBtn');
  ...
  public async goToDocuments() {
    if (!await (await this.backendConfigurationPnDocumentsButton()).isDisplayed()) {
  ...
  public async deleteDocumentSaveBtn() {
    const ele = await $('#deleteDocumentSaveBtn');
    await ele.waitForDisplayed({timeout: 40000});
    // await ele.waitForClickable({ timeout: 40000 });
    return ele;
  }

  public async deleteDocumentCancelBtn() {
    const ele = await $('#deleteDocumentCancelBtn');
    await ele.waitForDisplayed({timeout: 40000});
    await ele.waitForClickable({timeout: 40000});
    return ele;
  }

  public async documentEditSaveBtn() {
    const ele = await $('#documentEditSaveBtn');
    await ele.waitForDisplayed({timeout: 40000});
    // await ele.waitForClickable({ timeout: 40000 });
    return ele;
  }

  public async documentEditCancelBtn() {
    const ele = await $('#documentEditCancelBtn');
    await ele.waitForDisplayed({timeout: 40000});
    await ele.waitForClickable({timeout: 40000});
    return ele;
  }

  public async createDocumentCancelBtn() {
    const ele = await $('#createDocumentCancelBtn');
    await ele.waitForDisplayed({timeout: 40000});
    await ele.waitForClickable({timeout: 40000});
    return ele;
  }

  public async createDocumentSaveBtn() {
    const ele = await $('#createDocumentSaveBtn');
    await ele.waitForDisplayed({timeout: 40000});
    // await ele.waitForClickable({ timeout: 40000 });
    return ele;
  }
  ...
  public async editDocumentFolderSelector() { // it's selector. if you need input - add to query
    const ele = await $('#editDocumentFolderSelector');
    await ele.waitForDisplayed({timeout: 40000});
    // await ele.waitForClickable({ timeout: 40000 });
    return ele;
  }

  public async createDocumentFolderSelector() { // it's selector. if you need input - add to query
    const ele = await $('#createDocumentFolderSelector');
    await ele.waitForDisplayed({timeout: 40000});
    // await ele.waitForClickable({ timeout: 40000 });
    return ele;
  }
  ...
  public async openCreateFile(file: BackendFileCreate) {
    ...
    await (await this.createDocumentCancelBtn()).waitForClickable();
    if (file) {
  ...
  public async closeCreateFile(clickCancel = false) {
    if (clickCancel) {
      await (await this.createDocumentCancelBtn()).waitForClickable();
      await (await this.createDocumentCancelBtn()).click();
    } else {
      await (await this.createDocumentSaveBtn()).waitForClickable();
      await (await this.createDocumentSaveBtn()).click();
    }
  }
