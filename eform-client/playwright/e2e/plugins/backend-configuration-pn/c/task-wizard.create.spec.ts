...
test.describe('Area rules type 1', () => {
  ...
  test('should create always task', async ({ page }) => {
    ...
    const getPropertiesResponse = page.waitForResponse(
      r => r.url().includes('/api/backend-configuration-pn/task-wizard/properties?'),
      { timeout: 60000 }
    );
    await page.locator('#backend-configuration-pn-task-wizard').click();
    await getPropertiesResponse;
    ...
    await expect(page.locator('.cdk-row .cdk-column-folder span')).toHaveText(bindArea);
    ...
    await expect(page.locator('.cdk-row .cdk-column-repeat mat-chip span.mat-mdc-chip-action-label')).toHaveText(`${task.repeatType}`);
