...
describe('Area rules type 1', () => {
  ...
  it('should copy task', () => {
    ...
    cy.get('.cdk-row .cdk-column-eform span').eq(0).should('have.text', task.eformName + ' (13)');
    cy.get('.cdk-row .cdk-column-eform span').eq(1).should('have.text', task.eformName + ' (13)');
    ...
    cy.get('.cdk-row .cdk-column-eform span').eq(0).should('have.text', task.eformName + ' (13)');
    cy.get('.cdk-row .cdk-column-eform span').eq(1).should('have.text', task.eformName + ' (13)');
    cy.get('.cdk-row .cdk-column-eform span').eq(2).should('have.text', editedTask.eformName + ' (7)');
  });
  after(() => {
    backendConfigurationPropertiesPage.goToProperties();
    cy.wait(500);
