export const esES = {
  Actions: 'Comportamiento',
  'Admin only': 'Admin only',
  areas: 'áreas',
  Area: 'Área',
  'Read more': 'Leer más',
  'CHR Number': 'Número de CDH',
  'CVR Number': 'Número de CVR',
  Address: 'DIRECCIÓN',
  Languages: 'Idiomas',
  'View property areas': 'Ver áreas de propiedad',
  'Configure property areas': 'Configurar áreas de propiedad',
  'Edit property': 'Editar propiedad',
  'Delete property': 'Eliminar propiedad',
  'Create Property': 'Crear propiedad',
  'New property': 'Nueva propiedad',
  'Are you sure you want to delete': 'estas seguro que quieres borrarlo',
  'Check to enable': 'Marque para habilitar',
  Status: 'Estado',
  Select: 'Seleccionar',
  'Open area planning': 'Planificación de áreas abiertas',
  'New rule': 'Nueva regla',
  'Day of week': 'Día de la semana',
  Rule: 'Regla',
  'Selected template': 'Plantilla seleccionada',
  Open: 'Abierto',
  Closed: 'Cerrado',
  Type: 'Tipo',
  Yes: 'Sí',
  No: 'No',
  Alarm: 'Alarma',
  'Checklist stable': 'Lista de verificación estable',
  'Tail bite': 'mordedura de cola',
  Monday: 'Lunes',
  Tuesday: 'Martes',
  Wednesday: 'Miércoles',
  Thursday: 'Jueves',
  Friday: 'Viernes',
  Saturday: 'Sábado',
  Sunday: 'Domingo',
  'eForm name': 'nombre del formulario electrónico',
  'Edit rule': 'Editar regla',
  'Plan rule': 'regla del plan',
  'Notifications on/off': 'Notificaciones activadas/desactivadas',
  'Repeat every': 'Repite cada',
  Day: 'Día',
  Week: 'Semana',
  Month: 'Mes',
  'Repeat Type': 'Tipo de repetición',
  'Start date': 'Fecha de inicio',
  'End date': 'Fecha final',
  'Worker name': 'Nombre del trabajador',
  'Check to assign': 'Marque para asignar',
  'Create rule': 'Crear regla',
  ON: 'EN',
  OFF: 'APAGADO',
  Plan: 'Plan',
  'Delete rule': 'Eliminar regla',
  'Property name': 'Nombre de la propiedad',
  'Check to pair': 'Marque para emparejar',
  'Hours and energy': 'Horas y energía',
  Properties: 'Propiedades',
  Weekly: 'Semanalmente',
  '2nd week': '2da semana',
  '3rd week': '3ra semana',
  '4th week': '4ta semana',
  '5th week': '5ta semana',
  '6th week': '6ta semana',
  '7th week': 'séptima semana',
  '8th week': 'octava semana',
  '9th week': 'novena semana',
  '10th week': 'décima semana',
  '11st week': '11ra semana',
  '12nd week': '12da semana',
  '13rd week': '13ra semana',
  '14th week': 'semana 14',
  '15th week': 'semana 15',
  '16th week': '16a semana',
  '17th week': '17ma semana',
  '18th week': 'semana 18',
  '19th week': 'semana 19',
  '20th week': 'semana 20',
  '21st week': 'semana 21',
  '22nd week': 'semana 22',
  '23rd week': 'semana 23',
  '24th week': 'semana 24',
  '25th week': 'semana 25',
  '26th week': 'semana 26',
  '27th week': 'semana 27',
  '28th week': 'semana 28',
  '29th week': 'semana 29',
  '30th week': 'semana 30',
  '31st week': 'semana 31',
  '32nd week': '32a semana',
  '33rd week': 'semana 33',
  '34th week': 'semana 34',
  '35th week': 'semana 35',
  '36th week': 'semana 36',
  '37th week': 'semana 37',
  '38th week': 'semana 38',
  '39th week': 'semana 39',
  '40th week': 'semana 40',
  '41st week': 'semana 41',
  '42nd week': '42a semana',
  '43rd week': 'semana 43',
  '44th week': 'semana 44',
  '45th week': 'semana 45',
  '46th week': 'semana 46',
  '47th week': 'semana 47',
  '48th week': 'semana 48',
  '49th week': 'semana 49',
  '50th week': '50 semana',
  '51st week': 'semana 51',
  '52nd week': '52a semana',
  'Timeregistration enabled': 'Registro de tiempo habilitado',
  'Task management enabled': 'Gestión de tareas habilitada',
  'Select worker': 'Seleccionar trabajador',
  'Saved locally': 'Guardado localmente',
  'Ready for server': 'Listo para el servidor',
  'Received by server': 'Recibido por el servidor',
  'Retrieved by device': 'Recuperado por dispositivo',
  'Contact Microting': 'Error del sistema: Contacte con Microting',
  Name: 'Nombre',
  'Drop file(s) here or a choose from your computer': 'Suelte los archivos aquí o <u class="choose">elija</u> desde su computadora',
  'Create file': 'Crear archivo',
  'Download selected': 'Descargar seleccionado',
  'File name': 'Nombre del archivo',
  'Refresh table': 'Actualizar tabla',
  'Upload files': 'Subir archivos',
  'Edit file': 'Editar archivo',
  'Delete file': 'Eliminar archivo',
  Page: 'Página',
  'Zoom page': 'Acercar página',
  'Delete page': 'Eliminar página',
  'Create date': 'Fecha de creación',
  'View PDF': 'Ver PDF',
  'Edit file name': 'Editar nombre del archivo',
  'Edit tags file': 'Editar etiquetas de archivo',
  'Download selected files': 'Descargar archivos seleccionados',
  'Name ZIP-folder': 'Nombrar carpeta ZIP',
  Back: 'Atrás',
  'Download Document': 'Descargar {{language}} {{extension}}',
  'Select DOCX': 'Seleccione DOCX',
  'Select PDF': 'Seleccione PDF',
  'Every month': 'Cada mes',
  '2nd months': '2. mes',
  '3rd months': '3. mes',
  '6th months': '6. mes',
  '12 (1 year)': '12 (1 anio)',
  '24 (2 years)': '24 (2 años)',
  '36 (3 years)': '36 (3 años)',
  '48 (4 years)': '48 (4 años)',
  '60 (5 years)': '60 (5 años)',
  '72 (6 years)': '72 (6 años)',
  '84 (7 years)': '84 (7 años)',
  '96 (8 years)': '96 (8 años)',
  '108 (9 years)': '108 (9 años)',
  '120 (10 years)': '120 (10 años)',
  Repeat: 'Repetir',
  Deadline: 'Fecha límite',
  'Show/hide columns': 'Mostrar/ocultar columnas',
  GenerateWithTypeDoc: 'Generar {{tipo}}',
  copyFromTo: 'Copiar valores de {{from}} a {{to}}',
  'Add new task': 'Agregar nueva tarea',
  Folders: 'Carpetas',
  Property: 'Propiedad',
  Tags: 'Etiquetas',
  'Assign to': 'Asignar a',
  Active: 'Activo',
  NotActive: 'No activo',
  'Task name': 'Nombre de la tarea',
  'Task name is required': 'El nombre de la tarea es obligatorio',
  'Failed to rename task': 'No se pudo cambiar el nombre de la tarea',
  Folder: 'Carpeta',
  Notification: 'Notificación',
  'Planned tasks per': 'Tareas planificadas por',
  'Ad-hoc tasks per': 'Tareas ad hoc por',
  'Documents update per': 'Actualización de documentos por',
  'Task allocation, planned tasks per': 'Asignación de tareas, tareas planificadas por',
  'Task allocation, ad-hoc tasks per': 'Asignación de tareas, tareas ad hoc por',
  'Select property': 'Seleccionar propiedad',
  Exeeded: 'superado',
  Today: 'Hoy',
  '1-7 days': '1-7 días',
  '8-30 days': '8-30 días',
  'Over 30 days': 'Más de 30 días',
  'Report tag': 'Etiqueta de informe',
  'eForm template': 'plantilla de formulario electrónico',
  'Start from': 'Empezar desde',
  'Repeat type': 'Tipo de repetición',
  'System error: Contact Microting': 'Error del sistema: Contacte con Microting',
  'Create task': 'Crear tarea',
  'Folder not selected': 'Carpeta no seleccionada',
  'Select folder': 'Seleccione la carpeta',
  'Need to select property': 'Necesidad de seleccionar propiedad',
  All: 'Todo',
  'Last assigned to': 'Último asignado a',
  'Property - Areas': 'Propiedad - Áreas',
  Priority: 'Prioridad',
  'Created by': 'Creado por',
  'Created by text': 'Creado por texto',
  Show: 'Espectáculo',
  'Created date': 'Fecha de creación',
  'Last update date': 'Última Fecha de Actualización',
  'Last update by': 'Última actualización por',
  'Document name': 'Nombre del documento',
  Expiration: 'Vencimiento',
  'Folder name': 'Nombre de la carpeta',
  'Document description': 'Descripción del documento',
  'Manage folders': 'Administrar carpetas',
  'Create new document': 'Crear nuevo documento',
  'Create document': 'Crear documento',
  'Assign to property': 'Asignar a la propiedad',
  'Select/unselect all': 'Seleccionar/deseleccionar todo',
  'Name DOCX': 'Nombre DOCX',
  'Description DOCX': 'Descripción DOCX',
  'Name PDF': 'Nombre PDF',
  'Description PDF': 'Descripción PDF',
  'Selected folder': 'Carpeta seleccionada',
  'Folders management': 'Gestión de carpetas',
  'Edit folder': 'Editar carpeta',
  'Delete folder': 'Eliminar carpeta',
  'Edit document': 'Editar documento',
  'Delete document': 'Eliminar documento',
  Ongoing: 'En curso',
  Ordered: 'Ordenado',
  Awaiting: 'esperando',
  Urgent: 'Urgente',
  High: 'Alto',
  Medium: 'Medio',
  Low: 'Bajo',
  'No items found': 'No se encontraron artículos',
  'Select filter': 'Seleccionar filtro',
  'Generat report': 'generar informe',
  'Excel report': 'informe excel',
  'Create new task': 'Crear nueva tarea',
  'Property address': 'Dirección de Propiedad',
  'View compliance': 'Ver cumplimiento',
  'View areas': 'Ver áreas',
  'Edit list of areas for task management': 'Editar lista de áreas para la gestión de tareas',
  'Control area': 'Área de control',
  'Add controlareas to property': 'Agregar controleas a la propiedad',
  'CHR name': 'Nombre de la CDH',
  'CHR address': 'Dirección de la CDH',
  'Task management': 'Administración de tareas',
  'Assigned tasks': 'Tareas asignadas',
  Checkpoint: 'Control',
  'Show assignments': 'Mostrar tareas',
  Task: 'Tarea',
  Responsible: 'Responsable',
  'Word report': 'informe de palabra',
  'Expires today or has expired': 'Caduca hoy o ha caducado',
  'Expires in 1 month at the latest': 'Caduca a más tardar en 1 mes.',
  'Expires in more than 1 month': 'Caduca en más de 1 mes.',
  'Add bulk': 'Agregar masa',
  Names: 'Nombres',
  'Edit tag': 'Editar etiqueta',
  'Delete tag': 'Eliminar etiqueta',
  'Assigned to': 'Asignado a',
  'Task assigned to': 'Asignado a',
  'Edit task': 'Editar tarea',
  Every: 'Cada',
  'Copy task': 'Copiar tarea',
  'Delete task': 'Eliminar tarea',
  Exceeded: 'Excedido',
  'Exeeded or today': 'Excedido o hoy',
  'Under 30 days': 'Menos de 30 días',
  'Exceeded or today': 'Superado o hoy',
  'Grouping tags': 'Etiquetas de agrupación',
  'Generate report': 'Generar informe',
  'Task description': 'Descripción de la tarea',
  'Task solver': 'solucionador de tareas',
  'Description of task': 'Descripción de la tarea',
  'None found': 'Nada encontrado',
  'No tasks found': 'No se encontraron tareas',
  'No employees found': 'No se encontraron empleados',
  'No documents found': 'No se encontraron documentos',
  'No files found': 'No se encontraron archivos',
  'Create new employee': 'Crear nuevo empleado',
  'New employee': 'Nuevo empleado',
  'Edit employee': 'Editar empleado',
  Timeregistration: 'Registro de tiempo',
  'Submitted date': 'Fecha de envío',
  Always: 'Siempre',
  Completed: 'Terminado',
  Description: 'Descripción',
  Date: 'Fecha',
  Location: 'Ubicación',
  'Select date range': 'Seleccionar rango de fechas',
  'Manage tags': 'Administrar etiquetas',
  Middle: 'Medio',
  'Are you sure you want to permanently delete this case': '¿Está seguro de que desea eliminar permanentemente este caso?',
  Repeated: 'Repetido',
  'Upload PDF': 'Subir PDF',
  'PIN code': 'Código PIN',
  'Employee no': 'Numero de empleado',
  'Deactivate task': 'Desactivar tarea',
  Areas: 'Áreas',
  Pictures: 'Fotos',
  'Delete employee': 'Eliminar empleado',
  Resigned: 'Resignado',
  'Show resigned': 'Mostrar resignado',
  'Resigned at date': 'Dimitió en la fecha',
  'Show expired task on mobile': 'Mostrar tarea expirada en el móvil',
  Web: 'Web',
  Archive: 'Archivo',
  'Mobile timeregistration': 'Registro de tiempo móvil',
  'Phone number': 'Número de teléfono',
  'QR Code': 'Código QR',
  Close: 'Cerca',
  'Copied!': 'Copiado!',
  'Copy as image': 'Copiar como imagen',
  'Download PNG': 'Descargar PNG',
  'Break calculation': 'Cálculo de ruptura',
  'Mobile registration': 'Registro móvil',
  Shifts: 'Turnos',
  'Manager settings': 'Configuración del administrador',
  'Use Google Sheet as default': 'Usar Google Sheets como predeterminado',
  'Use only plan hours': 'Utilice únicamente las horas del plan.',
  'Auto break calculation': 'Cálculo de frenado automático',
  'Breaks are calculated automatically from the rule set. Manual break entry on shift tabs is disabled.': 'Los descansos se calculan automáticamente según las reglas establecidas. La introducción manual de descansos en las pestañas de turnos está deshabilitada.',
  'Allow time registration on mobile': 'Permitir el registro de tiempo en el móvil',
  'Main switch. When active, select registration method and editing policy below.': 'Interruptor principal. Cuando esté activo, seleccione el método de registro y la política de edición a continuación.',
  'How working time is captured': 'Cómo se registra el tiempo de trabajo',
  'Manual entry': 'Entrada manual',
  'The employee enters start, stop and break for each workday themselves.': 'El empleado introduce personalmente el inicio, el final y el descanso de cada jornada laboral.',
  'Use time clock on mobile': 'Utiliza el reloj de fichar en el móvil.',
  'Start/Stop buttons record work and breaks automatically as the day progresses.': 'Los botones de inicio/parada registran automáticamente el trabajo y los descansos a medida que avanza el día.',
  'Allow receipt for standard time on mobile': 'Permitir recepción para hora estándar en móvil',
  'Scheduled working hours can be approved with a single click. Scheduled working hours cannot be edited.': 'Las horas de trabajo programadas se pueden aprobar con un solo clic. Las horas de trabajo programadas no se pueden editar.',
  'Allow entry of forgotten days': 'Permitir la entrada de días olvidados',
  'The employee can retroactively register work hours they forgot to clock in/out on.': 'El empleado puede registrar retroactivamente las horas de trabajo que olvidó fichar al entrar o al salir.',
  'Shifts across midnight': 'Turnos que cruzan la medianoche',
  'The shift is automatically closed at 00:00 and continues as a new registration on the next day (e.g. day 1: 22:00–00:00, day 2: 00:00–06:00). The app handles this automatically.': 'El turno se cierra automáticamente a las 00:00 y continúa como un nuevo registro al día siguiente (p. ej., día 1: 22:00–00:00, día 2: 00:00–06:00). La aplicación lo gestiona automáticamente.',
  'May the employee edit past registrations?': '¿Puede el empleado editar registros anteriores?',
  'No editing allowed': 'No se permite la edición.',
  'Once a registration is saved, the employee can no longer change it. Errors must be corrected by a manager.': 'Una vez guardado un registro, el empleado ya no puede modificarlo. Los errores deben ser corregidos por un supervisor.',
  'Yes, until the last payroll run': 'Sí, hasta la última nómina.',
  'Locks on 19th': 'Cerraduras el día 19',
  'The employee can edit all registrations that fall after the last payroll run. Registrations up to and including the 19th of the month are locked when payroll runs. Older registrations cannot be changed.': 'El empleado puede editar todos los registros posteriores a la última nómina. Los registros hasta el día 19 del mes, inclusive, se bloquean al procesarse la nómina. Los registros anteriores no se pueden modificar.',
  '(Cutoff date is currently fixed at the 19th — will become configurable in a future version.)': '(Actualmente, la fecha límite está fijada para el día 19; se podrá configurar en una versión futura).',
  'Yes, but only the last 2 days': 'Sí, pero solo los últimos 2 días.',
  'The employee can only edit registrations within a rolling 2-day window. Everything older than 2 days is automatically locked, independent of payroll.': 'El empleado solo puede editar los registros dentro de un período continuo de 2 días. Todo lo que tenga más de 2 días de antigüedad se bloquea automáticamente, independientemente de la nómina.',
  'Enable third shift': 'Habilitar el tercer turno',
  'Enable fourth shift': 'Habilitar el cuarto turno',
  'Enable fifth shift': 'Habilitar el quinto turno',
  'Is manager': '¿Es gerente?',
  'Set this user as a manager': 'Establezca a este usuario como administrador.',
  'Managing tags': 'Gestionar etiquetas',
  'Select tags': 'Seleccionar etiquetas',
  'Select tags that this manager is responsible for': 'Seleccione las etiquetas de las que este gerente es responsable.',
  'Set password': 'Establecer contraseña',
  'Send reset password email': 'Enviar correo electrónico para restablecer la contraseña',
  'All day': 'Todo el día',
  List: 'Lista',
  Daily: 'A diario',
  'Every weekday': 'Todos los días laborables',
  Monthly: 'Mensual',
  Yearly: 'Anual',
  Custom: 'Costumbre',
  'Custom…': 'Costumbre…',
  'Does not repeat': 'No se repite',
  'Weekly on {{day}}': 'Semanalmente el {{day}}',
  'Monthly on day {{day}}': 'Mensualmente el día {{day}}',
  'Yearly on {{day}} {{month}}': 'Anualmente el {{día}} {{mes}}',
  Mon: 'Lun',
  Tue: 'Mar',
  Wed: 'Casarse',
  Thu: 'Jue',
  Fri: 'Vie',
  Sat: 'Se sentó',
  Sun: 'Sol',
  January: 'Enero',
  February: 'Febrero',
  March: 'Marzo',
  April: 'Abril',
  May: 'Puede',
  June: 'Junio',
  July: 'Julio',
  August: 'Agosto',
  September: 'Septiembre',
  October: 'Octubre',
  November: 'Noviembre',
  December: 'Diciembre',
  'Day(s)': 'Días)',
  'Week(s)': 'Semana(s)',
  'Month(s)': 'Meses)',
  'Year(s)': 'Años)',
  'Show details': 'Mostrar detalles',
  'Hide details': 'Ocultar detalles',
  Loading: 'Cargando',
  'No fields defined': 'No se han definido campos.',
  Copy: 'Copiar',
  'Copy of': 'Copia de',
  eForm: 'Formulario electrónico',
  'Report headline': 'titular del informe',
  Title: 'Título',
  'Set tags...': 'Establecer etiquetas...',
  'Assign to...': 'Asignar a...',
  'Add description': 'Añadir descripción',
  'My properties': 'Mis propiedades',
  'My teams': 'Mis equipos',
  'My employees': 'Mis empleados',
  'Create employee': 'Crear empleado',
  day: 'día',
  week: 'semana',
  month: 'mes',
  year: 'año',
  'Custom repeat': 'Repetición personalizada',
  'Repeat on': 'Repetir en',
  Ends: 'Fin',
  Never: 'Nunca',
  On: 'En',
  After: 'Después',
  occurrences: 'sucesos',
  Done: 'Hecho',
  'Could not save the event': 'No se pudo guardar el evento.',
  Error: 'Error',
  'At least one worker must be assigned': 'Se debe asignar al menos un trabajador.',
  'Previous week': 'Semana anterior',
  'Next week': 'La próxima semana',
  'Previous day': 'Día anterior',
  'Next day': 'Al día siguiente',
  'Could not create report headline': 'No se pudo crear el título del informe.',
  'Create new': 'Crear nuevo',
  'Change property': 'Cambiar propiedad',
  Wk: 'Semana',
  'Weekly every {{days}}': 'Semanalmente cada {{days}}',
  'Weekly on all days': 'Semanalmente todos los días',
  'Every {{n}} weeks: {{days}}': 'Cada {{n}} semanas: {{days}}',
  'Every {{n}} weeks: all days': 'Cada {{n}} semanas: todos los días',
  'Every {{n}} days': 'Cada {{n}} días',
  'Every {{n}} months on day {{dom}}': 'Cada {{n}} meses el día {{dom}}',
  'Every {{n}} years on {{dom}}. {{month}}': 'Cada {{n}} años en {{dom}}. {{month}}',
  'Attach file': 'Adjuntar archivo',
  'Save event first to attach files': 'Primero guarda el evento para adjuntar archivos.',
  'Delete attachment?': '¿Eliminar archivo adjunto?',
  'Could not download file': 'No se pudo descargar el archivo.',
  'Pending save': 'Pendiente de guardar',
  'Add Google Drive file': 'Agregar archivo de Google Drive',
  'Loading Google Picker': 'Cargando selector de Google',
  'Connect Google Drive': 'Conectar Google Drive',
  'Drive disconnected': 'Unidad desconectada',
  'Reconnect to resume sync': 'Vuelva a conectarse para reanudar la sincronización.',
  'Synced from Google Drive': 'Sincronizado desde Google Drive',
  'Could not connect to Google Drive': 'No se pudo conectar con Google Drive.',
  'Could not attach Drive file': 'No se pudo adjuntar el archivo de Drive.',
  'Google Drive connected': 'Google Drive conectado',
  'You can close this window': 'Puedes cerrar esta ventana.',
  'Open in Google Drive': 'Abrir en Google Drive',
  'Connected Google Drive accounts': 'Cuentas de Google Drive conectadas',
  'No Google Drive accounts connected.': 'No hay cuentas de Google Drive conectadas.',
  Disconnect: 'Desconectar',
  'Disconnect Google Drive?': '¿Desconectar Google Drive?',
  'Disconnecting will stop sync for any Drive-attached files.': 'Al desconectarse, se detendrá la sincronización de todos los archivos conectados a la unidad.',
  'Connected on': 'Conectados en',
  'Last used': 'Último uso',
  'Drive disconnected — reconnect to resume sync': 'Unidad desconectada: vuelva a conectarla para reanudar la sincronización.',
  'Last refreshed': 'Última actualización',
  'Google Drive disconnected': 'Google Drive desconectado',
  'Google Drive disconnected — reconnect to resume sync': 'Google Drive se ha desconectado; vuelva a conectarse para reanudar la sincronización.',
  'Ad hoc': 'A propósito',
  Time: 'Tiempo',
  'All weekdays (Monday to Friday)': 'Todos los días laborables (de lunes a viernes)',
  'Monthly on the {{ordinal}} {{day}}': 'Mensualmente en el {{ordinal}} {{día}}',
  'Every {{n}} months on the {{ordinal}} {{day}}': 'Cada {{n}} meses en el {{ordinal}} {{día}}',
  'Overdue task shown in app': 'Tarea vencida mostrada en la aplicación',
  'Overdue task not shown in app': 'Las tareas vencidas no se muestran en la aplicación.',
  'Repeat sub-type': 'Subtipo de repetición',
  'Monthly day': 'Día mensual',
  'Monthly on the first': 'Mensualmente el primero',
  Translate: 'Traducir',
  Farve: 'Color',
  'Tasks and actions': 'Tareas y acciones',
  Compliance: 'Cumplimiento',
  'Export CSV': 'Exportar CSV',
  Search: 'Buscar',
  Cancel: 'Cancelar',
  Create: 'Crear',
  'Team name': 'Nombre del equipo',
  'Delete team': 'Eliminar equipo',
  'Create team': 'Crear equipo',
  'Tag name': 'Nombre de la etiqueta',
  'Create tag': 'Crear etiqueta',
  'Enter tag name': 'Introduzca el nombre de la etiqueta',
  'Enter team name': 'Ingrese el nombre del equipo',
  Confirm: 'Confirmar',
  'Save changes': 'Guardar cambios',
  'Complete task': 'Completar la tarea',
  'Only this': 'Solo esto',
  'This and following': 'Esto y lo siguiente',
  'All in series': 'Todo en serie',
  'Date is required': 'Se requiere fecha',
  'Open link': 'Abrir enlace',
  'No worker found': 'No se encontró ningún trabajador.',
  'My tags': 'Mis etiquetas',
  ID: 'ID',
  Save: 'Ahorrar',
  Delete: 'Borrar',
  Edit: 'Editar',
  'Select date': 'Seleccione la fecha',
  'Edit repeat': 'Editar repetir',
  'Move repeat': 'Mover repetir',
  'Delete repeat': 'Eliminar repetir',
  'At least one worker or worker tag must be assigned': 'Se debe asignar al menos un trabajador o una etiqueta de trabajador.',
  'Assign to worker tags': 'Asignar a etiquetas de trabajador',
  'This and following (incl. completed)': 'Esto y lo siguiente (incluidos los completados)',
  'Payroll rules': 'Normas de nómina',
  'Pay Rule Set': 'Conjunto de reglas de pago',
  'Select the pay rule set for this worker': 'Seleccione el conjunto de reglas de pago para este trabajador.',
  'View pay rule set': 'Ver conjunto de reglas de pago',
  None: 'Ninguno',
  'Advanced settings': 'Configuración avanzada',
  'Use 1-minute intervals': 'Utilice intervalos de 1 minuto.',
  'Enable 1-minute granularity for time entry. Default increments are 5 or 15 minutes.': 'Habilite la granularidad de 1 minuto para el registro de tiempo. Los incrementos predeterminados son de 5 o 15 minutos.',
  'Once enabled, 1-minute intervals cannot be turned off': 'Una vez activados, los intervalos de 1 minuto no se pueden desactivar.',
  OK: 'OK',
  'My calendars': 'Mis calendarios',
  'Task visible on calendar and shown in app': 'La tarea es visible en el calendario y se muestra en la aplicación.',
  'Task dimmed on calendar and not shown in app': 'La tarea aparece atenuada en el calendario y no se muestra en la aplicación.',
  Calendar: 'Calendario',
  'Create calendar': 'Crear calendario',
  'Calendar name': 'Nombre del calendario',
  'Delete calendar': 'Eliminar calendario',
  'This will permanently delete the calendar and its {{count}} events. This cannot be undone.': 'Esto eliminará permanentemente el calendario y sus {{count}} eventos. Esta acción no se puede deshacer.',
  'Enter calendar name': 'Introduzca el nombre del calendario',
  'Are you sure you want to delete the calendar': '¿Estás seguro de que quieres eliminar el calendario?',
  'All tasks associated with this calendar will lose their calendar association': 'Todas las tareas asociadas a este calendario perderán su asociación con el calendario.',
  'All properties': 'Todas las propiedades',
  'All calendars': 'Todos los calendarios',
  'All tags': 'Todas las etiquetas',
  'All employees': 'Todos los empleados',
  'Not completed tasks': 'Tareas no completadas',
  'Completed tasks': 'Tareas completadas',
  'All tasks': 'Todas las tareas',
  '1 month': '1 mes',
  '3 months': '3 meses',
  '6 months': '6 meses',
  '12 months': '12 meses',
  'Year to date': 'El año hasta la fecha',
  'Set period': 'Período establecido',
  'Show report': 'Mostrar informe',
  'Download as': 'Descargar como',
  Download: 'Descargar',
  'Show all': 'Mostrar todo',
  Showing: 'Demostración',
  'Select filters and click Show report': 'Seleccione los filtros y haga clic en Mostrar informe.',
  'No tasks in the selected period': 'No hay tareas en el período seleccionado.',
  'Delete log': 'Eliminar registro',
  'This action cannot be undone': 'Esta acción no se puede deshacer.',
  Employees: 'Empleados',
  At: 'En',
  'Time of day': 'Hora del día',
  'Done date': 'Fecha de finalización',
  'Completed by': 'Completado por',
  View: 'Vista',
  'Task visible on calendar': 'Tarea visible en el calendario',
  'Task dimmed on calendar': 'La tarea aparece atenuada en el calendario.',
  'This task is part of a recurring series. What do you want to edit?': 'Esta tarea forma parte de una serie recurrente. ¿Qué quieres editar?',
  'This task is part of a recurring series. What do you want to move?': 'Esta tarea forma parte de una serie recurrente. ¿Qué quieres mover?',
  'This task is part of a recurring series. What do you want to delete?': 'Esta tarea forma parte de una serie recurrente. ¿Qué desea eliminar?',
  'Updated.': 'Actualizado.',
  'Select / create report headline': 'Seleccione / cree el título del informe',
  'No tasks this month': 'No hay tareas este mes',
  'Previous month': 'Mes anterior',
  'Next month': 'Mes próximo',
  'No tasks this week': 'Esta semana no hay tareas',
  Id: 'Identificación',
  Action: 'Acción',
  Employee: 'Empleado',
  'tasks selected': 'tareas seleccionadas',
  'Show paginated': 'Mostrar paginado',
  'From employee': 'De parte del empleado',
  'To employee': 'Para emplear',
  'Move selected to employee': 'Trasladar seleccionado a empleado',
  'Move from employee to employee': 'Pasar de un empleado a otro',
  'Add employee': 'Agregar empleado',
  'Change eForm': 'Modificar formulario electrónico',
  'The eForm will be changed immediately for the selected tasks': 'El formulario electrónico se modificará inmediatamente para las tareas seleccionadas.',
  'Add tags': 'Agregar etiquetas',
  'Remove tags': 'Eliminar etiquetas',
  'Set compliance': 'Establecer cumplimiento',
  'Copy to property': 'Copiar a la propiedad',
  'Delete selected': 'Eliminar seleccionado',
  'Collected data will not be deleted': 'Los datos recopilados no serán eliminados.',
  Tasks: 'Tareas',
  'Batch action': 'Acción por lotes',
  'Select batch action': 'Seleccione la acción por lotes',
  'Delete photo': 'Eliminar foto',
  'Delete photo?': '¿Eliminar foto?',
  'The photo will be removed from the task. This action cannot be undone.':
    'La foto se eliminará de la tarea. Esta acción no se puede deshacer.',
  'The eForm applies to the whole series. Changing it updates every occurrence that has not been completed yet, including occurrences in the past. Completed occurrences keep the eForm they were completed with.': 'El formulario electrónico se aplica a toda la serie. Al modificarlo, se actualizan todas las instancias que aún no se han completado, incluidas las anteriores. Las instancias completadas conservan el formulario electrónico con el que se completaron.',
  'Select a calendar': 'Seleccione un calendario',
  'Assigned workers': 'Trabajadores asignados',
  'Other workers': 'Otros trabajadores',
  Sections: 'Secciones',
  // Task list batch: change start date (#1122). The preview panel's four
  // counts are one key each so each language can word-order them freely.
  'Change start date': 'Cambiar la fecha de inicio',
  '{{count}} tasks': '{{count}} tareas',
  '{{count}} open occurrences will be retracted': '{{count}} ocurrencias abiertas se retirarán',
  '{{count}} completed will be preserved': '{{count}} completadas se conservarán',
  '{{count}} overdue tasks will be created': '{{count}} tareas vencidas se crearán',
  'Calculating preview': 'Calculando la vista previa',
  'Preview could not be loaded': 'No se pudo cargar la vista previa',
  'Pick a start date to preview the change': 'Elija una fecha de inicio para ver la vista previa del cambio',
  // Task list batch: activate / deactivate (#1123). The two radio labels
  // reuse the existing calendar-modal status keys; only the action label
  // and the honest deactivate warning are new.
  'Activate / deactivate': 'Activar / desactivar',
  'Deactivating retracts the open occurrences from the app. Completed occurrences and the data already collected are preserved.': 'La desactivación retira de la aplicación las repeticiones abiertas. Las repeticiones completadas y los datos ya recopilados se conservan.',
  'Update table': 'Actualizar tabla',
  'Select filters and click Update table': 'Seleccione los filtros y haga clic en Actualizar tabla.',
  'Compliance overview': 'Descripción general',
  'Compliance details': 'Detalles',
  'Compliance report': 'Informe',
  'Overview shows both completed and not completed tasks': 'La vista general muestra tanto las tareas completadas como las no completadas.',
  'Select a property to filter by calendar': 'Seleccione una propiedad para filtrar por calendario',
  'Select both a start and an end date': 'Seleccione una fecha de inicio y una fecha de finalización.',
  'The end date must be on or after the start date': 'La fecha de finalización debe ser igual o posterior a la fecha de inicio.',
  'Showing all': 'Mostrando todos',
  'No results': 'No hay resultados',
  Filtering: 'Filtración',
  'View mode': 'Modo de visualización',
  Pagination: 'Paginación',
  'No tasks match the selected filters.': 'No se encontraron tareas que coincidan con los filtros seleccionados.',
  'Task done': 'Tarea realizada',
  'Task not done': 'Tarea no realizada',
  'Could not load the data': 'No se pudieron cargar los datos',
  // Backfill of keys present in enUS.ts but missing here (#1167).
  // Machine-translated via translateTsFiles.py; review welcome.
  'Adhoc overview': 'Resumen ad hoc',
  Overview: 'Descripción general',
  History: 'Historia',
  'New task': 'Nueva tarea',
  Content: 'Contenido',
  'Last deadline': 'Último plazo',
  Overdue: 'Atrasado',
  Solved: 'Resuelto',
  Archived: 'Archivado',
  'With comments': 'Con comentarios',
  'Without comments': 'Sin comentarios',
  'Choose who completes the task': 'Elige quién completa la tarea.',
  'Task can be seen and completed by': 'La tarea puede ser vista y completada por',
  'Assigned only': 'Asignado únicamente',
  Everyone: 'Todos',
  'Execute no later than and first display': 'Ejecutar a más tardar y mostrar primero',
  'Last 30 days': 'Últimos 30 días',
  'Last 60 days': 'Últimos 60 días',
  'Last 90 days': 'Últimos 90 días',
  'Last 6 months': 'Últimos 6 meses',
  'Last 12 months': 'Últimos 12 meses',
  'Last 24 months': 'Últimos 24 meses',
  'Custom period': 'Período personalizado',
  Columns: 'Columnas',
  Created: 'Creado',
  'No deadline': 'Sin fecha límite',
  'Deadline today': 'Fecha límite hoy',
  '{{count}} days until deadline': '{{count}} días hasta la fecha límite',
  And: 'Y',
  Or: 'O',
  Weekdays: 'días laborables',
  'Deadline reminder': 'Recordatorio de fecha límite',
  'First visibility reminder': 'Recordatorio de primera visibilidad',
  Comments: 'Comentarios',
  Period: 'Período',
  Filters: 'Filtros',
  From: 'De',
  To: 'A',
  'Archive task': 'Tarea de archivo',
  created: 'Creado',
  assigned: 'Asignado',
  completed: 'Terminado',
  archived: 'Archivado',
  commented: 'Comentado',
  '30 days': '30 días',
  '60 days': '60 días',
  '90 days': '90 días',
  '6 months period': '6 meses',
  '12 months period': '12 meses',
  'Period and filter': 'Periodo y filtro',
  'All areas': 'Todas las áreas',
  'Select a property to filter by area': 'Seleccione una propiedad para filtrar por área.',
  'Tags in history (AND)': 'Etiquetas en la historia (Y)',
  'Search tags': 'Etiquetas de búsqueda…',
  'No tags match the search': 'No se encontraron etiquetas que coincidan con la búsqueda.',
  'No tags yet - create them under Overview': 'Aún no hay etiquetas; créalas en la sección Descripción general.',
  'Remove tag {{name}}': 'Eliminar «{{name}}»',
  'None selected': 'Ninguno seleccionado',
  'Active period': 'Período activo',
  '{{from}} to {{to}} inclusive of both days': '{{from}} — {{to}} (ambos días incluidos).',
  'History completed tasks': 'Historial: tareas completadas',
  'Completed date': 'Terminado',
  'Task resolved status': 'Resuelto',
  'No tasks match the selected period, property, area and tags': 'No se encontraron tareas que coincidan con el período, la propiedad, el área y las etiquetas seleccionadas.',
  'Previous page': 'Anterior',
  'Next page': 'Próximo',
  'Page {{page}} of {{pages}}': 'Página {{page}} de {{pages}}',
  hit: 'golpear',
  hits: 'golpes',
  'Show help text': 'Mostrar texto de ayuda',
  'Hide help text': 'Ocultar texto de ayuda',
  'Completion date term': 'Fecha de finalización',
  'is the date of': 'es la fecha de',
  'when the task was completed': '(cuando se completó la tarea).',
  is: 'es',
  'inclusive of both ends': 'incluyendo ambos extremos',
  'whole days by calendar date': '(días completos por fecha del calendario).',
  'Fixed periods are counted backwards from': 'Los períodos fijos se cuentan hacia atrás desde',
  'Day presets term': '30 / 60 / 90 días',
  'equal the stated number of calendar days including today': '= el número indicado de días naturales, incluido el día de hoy.',
  and: 'y',
  'equal the same calendar day shifted back': '= el mismo día del calendario desplazado hacia atrás ese número de meses (ajustado si el día no existe en ese mes).',
  'below the active period line narrow the list to': 'debajo de la línea «Período activo:» limite la lista a la propiedad seleccionada y, una vez seleccionada una propiedad, a un área específica o a',
  'all areas term': 'todas las áreas',
  'on that property': 'en esa propiedad.',
  'The area dropdown is only active once a specific property is selected': 'El menú desplegable de áreas solo se activa una vez que haya seleccionado una propiedad específica (no «Todas las propiedades»).',
  'Tags AND term': 'Etiquetas (Y)',
  'when one or more tags are selected only tasks that have': ': cuando se selecciona una o más etiquetas, solo las tareas que tengan',
  'all term': 'todo',
  'selected tags are shown without selected tags every completed task in the period is included': 'Se muestran las etiquetas seleccionadas. Si no se seleccionan etiquetas, se incluyen todas las tareas completadas durante el período.',
  'can be hidden or shown with the header line': 'puede ocultarse o mostrarse con la línea de encabezado (predeterminado: cerrado). El grupo reúne',
  Add: 'Agregar',
  'Area name is empty or already exists': 'El nombre del área está vacío o ya existe.',
  'Create areas': 'Crear áreas',
  'Create one or more areas on the selected property (one name per line)': 'Cree una o más áreas en la propiedad seleccionada (un nombre por línea).',
  'Delete area': 'Eliminar área',
  'Edit or delete areas for the selected property': 'Edita o elimina áreas de la propiedad seleccionada.',
  'Existing tasks keep their history': 'Las tareas existentes conservan su historial.',
  'Failed to create areas': 'No se pudieron crear áreas',
  'Failed to delete area': 'No se pudo eliminar el área',
  'Manage areas': 'Gestionar áreas',
  'No areas on this property yet': 'Aún no hay zonas definidas en esta propiedad.',
  'One area name per line': 'Un nombre de área por línea.',
  'Rename area': 'Cambiar el nombre del área',
  'Compliance %': 'Cumplimiento %',
  'Compliance total': 'Total',
  'No properties match the selected filters.': 'Ninguna propiedad coincide con los filtros seleccionados.',
  'Sort by': 'Ordenar por',
  'Completion date': 'Fecha de finalización',
  'Without tag': 'Sin etiqueta',
  'Columns unavailable': 'Columnas no disponibles',
  '1 image': '1 imagen',
  '{{count}} images': '{{count}} imágenes',
  'Only completed tasks can be edited': 'Solo se pueden editar las tareas completadas.',
};
