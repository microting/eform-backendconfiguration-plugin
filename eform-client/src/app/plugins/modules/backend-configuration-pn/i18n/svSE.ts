export const svSE = {
  Actions: 'Handlingar',
  'Admin only': 'Admin only',
  areas: 'områden',
  Area: 'Område',
  'Read more': 'Läs mer',
  'CHR Number': 'CHR-nummer',
  'CVR Number': 'CVR-nummer',
  Address: 'Adress',
  Languages: 'Språk',
  'View property areas': 'Visa fastighetsområden',
  'Configure property areas': 'Konfigurera fastighetsområden',
  'Edit property': 'Redigera fastighet',
  'Delete property': 'Ta bort fastighet',
  'Create Property': 'Skapa fastighet',
  'New property': 'Ny fastighet',
  'Are you sure you want to delete': 'Är du säker på att du vill ta bort',
  'Check to enable': 'Kontrollera för att aktivera',
  Status: 'Status',
  Select: 'Välj',
  'Open area planning': 'Öppna områdesplanering',
  'New rule': 'Ny regel',
  'Day of week': 'Veckodag',
  Rule: 'Regel',
  'Selected template': 'Vald mall',
  Open: 'Öppen',
  Closed: 'Stängd',
  Type: 'Typ',
  Yes: 'Ja',
  No: 'Nej',
  Alarm: 'Larm',
  'Checklist stable': 'Kontrolllista stables',
  'Tail bite': 'Halebit',
  Monday: 'Måndag',
  Tuesday: 'Tisdag',
  Wednesday: 'Onsdag',
  Thursday: 'Torsdag',
  Friday: 'Fredag',
  Saturday: 'Lördag',
  Sunday: 'Söndag',
  'eForm name': 'eForm namn',
  'Edit rule': 'Redigera regel',
  'Plan rule': 'Planeringsregel',
  'Notifications on/off': 'Meddelanden på/av',
  'Repeat every': 'Upprepa varje',
  Day: 'Dag',
  Week: 'Vecka',
  Month: 'Månad',
  'Repeat Type': 'Upprepa typ',
  'Start date': 'Startdatum',
  'End date': 'Slutdatum',
  'Worker name': 'Arbetarnamn',
  'Check to assign': 'Kontrollera för att tilldela',
  'Create rule': 'Skapa regel',
  ON: 'PÅ',
  OFF: 'AV',
  Plan: 'Planera',
  'Delete rule': 'Ta bort regel',
  'Property name': 'Fastighetsnamn',
  'Check to pair': 'Kontrollera för att parra',
  'Hours and energy': 'Timmar och energi',
  Properties: 'Fastigheter',
  Weekly: 'Veckovis',
  '2nd week': '2:a veckan',
  '3rd week': '3:e veckan',
  '4th week': '4:e veckan',
  '5th week': '5:e veckan',
  '6th week': '6:e veckan',
  '7th week': '7:e veckan',
  '8th week': '8:e veckan',
  '9th week': '9:e veckan',
  '10th week': '10:e veckan',
  '11st week': '11:e veckan',
  '12nd week': '12:e veckan',
  '13rd week': '13:e veckan',
  '14th week': '14:e veckan',
  '15th week': '15:e veckan',
  '16th week': '16:e veckan',
  '17th week': '17:e veckan',
  '18th week': '18:e veckan',
  '19th week': '19:e veckan',
  '20th week': '20:e veckan',
  '21st week': '21:e veckan',
  '22nd week': '22:e veckan',
  '23rd week': '23:e veckan',
  '24th week': '24:e veckan',
  '25th week': '25:e veckan',
  '26th week': '26:e veckan',
  '27th week': '27:e veckan',
  '28th week': '28:e veckan',
  '29th week': '29:e veckan',
  '30th week': '30:e veckan',
  '31st week': '31:e veckan',
  '32nd week': '32:e veckan',
  '33rd week': '33:e veckan',
  '34th week': '34:e veckan',
  '35th week': '35:e veckan',
  '36th week': '36:e veckan',
  '37th week': '37:e veckan',
  '38th week': '38:e veckan',
  '39th week': '39:e veckan',
  '40th week': '40:e veckan',
  '41st week': '41:e veckan',
  '42nd week': '42:e veckan',
  '43rd week': '43:e veckan',
  '44th week': '44:e veckan',
  '45th week': '45:e veckan',
  '46th week': '46:e veckan',
  '47th week': '47:e veckan',
  '48th week': '48:e veckan',
  '49th week': '49:e veckan',
  '50th week': '50:e veckan',
  '51st week': '51:e veckan',
  '52nd week': '52:e veckan',
  'Timeregistration enabled': 'Tidregistrering aktiverad',
  'Task management enabled': 'Uppgiftshantering aktiverad',
  'Select worker': 'Välj arbetare',
  'Saved locally': 'Sparad lokalt',
  'Ready for server': 'Klar för server',
  'Received by server': 'Mottagen av server',
  'Retrieved by device': 'Hämtad av enhet',
  'Contact Microting': 'Systemfel: Kontakta Microting',
  Name: 'Namn',
  'Drop file(s) here or a choose from your computer': 'Släpp filer här eller <u class="choose">välj</u> från din dator',
  'Create file': 'Skapa fil',
  'Download selected': 'Ladda ner vald',
  'File name': 'Filnamn',
  'Refresh table': 'Uppdatera tabell',
  'Upload files': 'Ladda upp filer',
  'Edit file': 'Redigera fil',
  'Delete file': 'Ta bort fil',
  Page: 'Sida',
  'Zoom page': 'Zooma sida',
  'Delete page': 'Ta bort sida',
  'Create date': 'Skapad datum',
  'View PDF': 'Visa PDF',
  'Edit file name': 'Redigera filnamn',
  'Edit tags file': 'Redigera filtagg',
  'Download selected files': 'Ladda ner valda filer',
  'Name ZIP-folder': 'Namn ZIP-mapp',
  Back: 'Tillbaka',
  'Download Document': 'Ladda ner {{language}} {{extension}}',
  'Select DOCX': 'Välj DOCX',
  'Select PDF': 'Välj PDF',
  'Every month': 'Varje månad',
  '2nd months': '2. måned',
  '3rd months': '3. måned',
  '6th months': '6. måned',
  '12 (1 year)': '12 (1 år)',
  '24 (2 years)': '24 (2 år)',
  '36 (3 years)': '36 (3 år)',
  '48 (4 years)': '48 (4 år)',
  '60 (5 years)': '60 (5 år)',
  '72 (6 years)': '72 (6 år)',
  '84 (7 years)': '84 (7 år)',
  '96 (8 years)': '96 (8 år)',
  '108 (9 years)': '108 (9 år)',
  '120 (10 years)': '120 (10 år)',
  Repeat: 'Upprepa',
  Deadline: 'Deadline',
  'Show/hide columns': 'Visa/dölj kolumner',
  GenerateWithTypeDoc: 'Generera {{type}}',
  copyFromTo: 'Kopiera från {{from}} till {{to}}',
  'Add new task': 'Lägg till ny uppgift',
  Folders: 'Mappar',
  Property: 'Fast egendom',
  Tags: 'Taggar',
  'Assign to': 'Tilldela till',
  Active: 'Aktiv',
  NotActive: 'Inte aktiv',
  'Task name': 'Uppgiftsnamn',
  'Task name is required': 'Uppgiftsnamn krävs',
  'Failed to rename task': 'Det gick inte att byta namn på uppgiften',
  Folder: 'Mapp',
  Notification: 'Meddelande',
  'Planned tasks per': 'Planerade arbetsuppgifter per',
  'Ad-hoc tasks per': 'Ad hoc arbetsuppgifter per',
  'Documents update per': 'Dokumentuppdatering per',
  'Task allocation, planned tasks per': 'Arbetsfördelning, planerade uppgifter per',
  'Task allocation, ad-hoc tasks per': 'Arbetsfördelning, ad-hoc uppgifter per',
  'Select property': 'Välj egenskap',
  Exeeded: 'Exeeded',
  Today: 'I dag',
  '1-7 days': '1-7 dagar',
  '8-30 days': '8-30 dagar',
  'Over 30 days': 'Över 30 dagar',
  'Report tag': 'Rapportera tagg',
  'eForm template': 'eForm mall',
  'Start from': 'Börja från',
  'Repeat type': 'Upprepa typ',
  'System error: Contact Microting': 'Systemfel: Kontakta Microting',
  'Create task': 'Skapa uppgift',
  'Folder not selected': 'Mapp inte vald',
  'Select folder': 'Välj mapp',
  'Need to select property': 'Behöver välja egendom',
  All: 'Allt',
  'Last assigned to': 'Senast tilldelad till',
  'Property - Areas': 'Fastighet - Ytor',
  Priority: 'Prioritet',
  'Created by': 'Skapad av',
  'Created by text': 'Skapad av text',
  Show: 'Show',
  'Created date': 'Skapat Datum',
  'Last update date': 'Senaste uppdateringsdatum',
  'Last update by': 'Senaste uppdatering av',
  'Document name': 'Dokument namn',
  Expiration: 'Utgång',
  'Folder name': 'Mapp namn',
  'Document description': 'Dokumentbeskrivning',
  'Manage folders': 'Hantera mappar',
  'Create new document': 'Skapa nytt dokument',
  'Create document': 'Skapa dokument',
  'Assign to property': 'Tilldela till egendom',
  'Select/unselect all': 'Markera/avmarkera alla',
  'Name DOCX': 'Namn DOCX',
  'Description DOCX': 'Beskrivning DOCX',
  'Name PDF': 'Namn PDF',
  'Description PDF': 'Beskrivning PDF',
  'Selected folder': 'Vald mapp',
  'Folders management': 'Mapparhantering',
  'Edit folder': 'Redigera mapp',
  'Delete folder': 'Ta bort mapp',
  'Edit document': 'Redigera dokument',
  'Delete document': 'Ta bort dokument',
  Ongoing: 'Pågående',
  Ordered: 'Beordrade',
  Awaiting: 'Avvaktan',
  Urgent: 'Brådskande',
  High: 'Hög',
  Medium: 'Medium',
  Low: 'Låg',
  'No items found': 'Inga föremål hittades',
  'Select filter': 'Välj filter',
  'Generat report': 'Generera rapport',
  'Excel report': 'Excel-rapport',
  'Create new task': 'Skapa ny uppgift',
  'Property address': 'Fastighetsadress',
  'View compliance': 'Visa efterlevnad',
  'View areas': 'Visa områden',
  'Edit list of areas for task management': 'Redigera lista över områden för uppgiftshantering',
  'Control area': 'Kontrollområde',
  'Add controlareas to property': 'Lägg till kontrollområden till egenskapen',
  'CHR name': 'CHR namn',
  'CHR address': 'CHR-adress',
  'Task management': 'Uppgiftshantering',
  'Assigned tasks': 'Tilldelade uppgifter',
  Checkpoint: 'Kontrollstation',
  'Show assignments': 'Visa uppdrag',
  Task: 'Uppgift',
  Responsible: 'Ansvarig',
  'Word report': 'Ordrapport',
  'Expires today or has expired': 'Går ut idag eller har gått ut',
  'Expires in 1 month at the latest': 'Går ut senast om 1 månad',
  'Expires in more than 1 month': 'Går ut om mer än 1 månad',
  'Add bulk': 'Lägg till bulk',
  Names: 'Namn',
  'Edit tag': 'Redigera tagg',
  'Delete tag': 'Ta bort tagg',
  'Assigned to': 'Tilldelats',
  'Task assigned to': 'Tilldelats',
  'Edit task': 'Redigera uppgift',
  Every: 'Varje',
  'Copy task': 'Kopiera uppgift',
  'Delete task': 'Ta bort uppgift',
  Exceeded: 'Överskred',
  'Exeeded or today': 'Exeeded eller idag',
  'Under 30 days': 'Under 30 dagar',
  'Exceeded or today': 'Överskriden eller idag',
  'Grouping tags': 'Gruppering av taggar',
  'Generate report': 'Generera rapport',
  'Task description': 'Uppgifts beskrivning',
  'Task solver': 'Uppgiftslösare',
  'Description of task': 'Beskrivning av uppgiften',
  'None found': 'Ingen hittades',
  'No tasks found': 'Inga uppgifter hittades',
  'No employees found': 'Inga anställda hittades',
  'No documents found': 'Inga dokument hittades',
  'No files found': 'Inga filer funna',
  'Create new employee': 'Skapa ny medarbetare',
  'New employee': 'Ny anställd',
  'Edit employee': 'Redigera medarbetare',
  Timeregistration: 'Tidregistrering',
  'Submitted date': 'Inlämningsdatum',
  Always: 'Alltid',
  Completed: 'Avslutad',
  Description: 'Beskrivning',
  Date: 'Datum',
  Location: 'Plats',
  'Select date range': 'Välj datumintervall',
  'Manage tags': 'Hantera taggar',
  Middle: 'Mitten',
  'Are you sure you want to permanently delete this case': 'Är du säker på att du vill ta bort det här ärendet permanent',
  Repeated: 'Upprepad',
  'Upload PDF': 'Ladda upp PDF',
  'PIN code': 'Pinkod',
  'Employee no': 'Anställningsnummer',
  'Deactivate task': 'Inaktivera uppgiften',
  Areas: 'Områden',
  Pictures: 'Bilder',
  'Delete employee': 'Ta bort anställd',
  Resigned: 'Resignerad',
  'Show resigned': 'Visa avgick',
  'Resigned at date': 'Avgick vid datumet',
  'Show expired task on mobile': 'Visa utgången uppgift på mobilen',
  Web: 'Web',
  Archive: 'Arkiv',
  'Ad hoc': 'Ad hoc',
  Time: 'Tid',
  'Mobile timeregistration': 'Mobil tidregistrering',
  'Phone number': 'Telefonnummer',
  'QR Code': 'QR-kod',
  Close: 'Nära',
  'Copied!': 'Kopierad!',
  'Copy as image': 'Kopiera som bild',
  'Download PNG': 'Ladda ner PNG',
  'Break calculation': 'Pausberäkning',
  'Mobile registration': 'Mobil registrering',
  Shifts: 'Skift',
  'Manager settings': 'Inställningar för chef',
  'Use Google Sheet as default': 'Använd Google Sheet som standard',
  'Use only plan hours': 'Använd endast planerade timmar',
  'Auto break calculation': 'Automatisk rastberäkning',
  'Breaks are calculated automatically from the rule set. Manual break entry on shift tabs is disabled.': 'Raster beräknas automatiskt från regeluppsättningen. Manuell rastinmatning på skiftflikar är inaktiverad.',
  'Allow time registration on mobile': 'Tillåt tidsregistrering på mobilen',
  'Main switch. When active, select registration method and editing policy below.': 'Huvudströmbrytare. När den är aktiv, välj registreringsmetod och redigeringspolicy nedan.',
  'How working time is captured': 'Hur arbetstid registreras',
  'Manual entry': 'Manuell inmatning',
  'The employee enters start, stop and break for each workday themselves.': 'Den anställde anger själv start, stopp och rast för varje arbetsdag.',
  'Use time clock on mobile': 'Använd stämpelklocka på mobilen',
  'Start/Stop buttons record work and breaks automatically as the day progresses.': 'Start/stopp-knappar registrerar arbete och pauser automatiskt allt eftersom dagen fortskrider.',
  'Allow receipt for standard time on mobile': 'Tillåt kvitto för standardtid på mobilen',
  'Scheduled working hours can be approved with a single click. Scheduled working hours cannot be edited.': 'Schemalagda arbetstider kan godkännas med ett enda klick. Schemalagda arbetstider kan inte redigeras.',
  'Allow entry of forgotten days': 'Tillåt inmatning av glömda dagar',
  'The employee can retroactively register work hours they forgot to clock in/out on.': 'Medarbetaren kan retroaktivt registrera arbetstimmar som de glömt att stämpla in/ut på.',
  'Shifts across midnight': 'Skift över midnatt',
  'The shift is automatically closed at 00:00 and continues as a new registration on the next day (e.g. day 1: 22:00–00:00, day 2: 00:00–06:00). The app handles this automatically.': 'Skiftet stängs automatiskt kl. 00:00 och fortsätter som en ny registrering nästa dag (t.ex. dag 1: 22:00–00:00, dag 2: 00:00–06:00). Appen hanterar detta automatiskt.',
  'May the employee edit past registrations?': 'Får den anställde redigera tidigare registreringar?',
  'No editing allowed': 'Ingen redigering tillåten',
  'Once a registration is saved, the employee can no longer change it. Errors must be corrected by a manager.': 'När en registrering har sparats kan medarbetaren inte längre ändra den. Fel måste korrigeras av en chef.',
  'Yes, until the last payroll run': 'Ja, fram till sista löneutbetalningen',
  'Locks on 19th': 'Lås på 19:e',
  'The employee can edit all registrations that fall after the last payroll run. Registrations up to and including the 19th of the month are locked when payroll runs. Older registrations cannot be changed.': 'Medarbetaren kan redigera alla registreringar som infaller efter den senaste lönekörningen. Registreringar fram till och med den 19:e i månaden är låsta vid lönekörning. Äldre registreringar kan inte ändras.',
  '(Cutoff date is currently fixed at the 19th — will become configurable in a future version.)': '(Sista datum är för närvarande fastställt till den 19:e – kommer att bli konfigurerbart i en framtida version.)',
  'Yes, but only the last 2 days': 'Ja, men bara de senaste 2 dagarna',
  'The employee can only edit registrations within a rolling 2-day window. Everything older than 2 days is automatically locked, independent of payroll.': 'Medarbetaren kan bara redigera registreringar inom ett rullande 2-dagarsfönster. Allt som är äldre än 2 dagar låses automatiskt, oberoende av lönehantering.',
  'Enable third shift': 'Aktivera tredje skiftet',
  'Enable fourth shift': 'Aktivera fjärde skiftet',
  'Enable fifth shift': 'Aktivera femte skiftet',
  'Is manager': 'Är chef',
  'Set this user as a manager': 'Ange den här användaren som administratör',
  'Managing tags': 'Hantera taggar',
  'Select tags': 'Välj taggar',
  'Select tags that this manager is responsible for': 'Välj taggar som den här hanteraren ansvarar för',
  'Set password': 'Ange lösenord',
  'Send reset password email': 'Skicka e-post om återställning av lösenord',
  'All day': 'Hela dagen',
  List: 'Lista',
  Daily: 'Dagligen',
  'Every weekday': 'Varje vardag',
  Monthly: 'Månatlig',
  Yearly: 'Årlig',
  Custom: 'Beställnings',
  'Custom…': 'Beställnings…',
  'Does not repeat': 'Upprepas inte',
  'Weekly on {{day}}': 'Veckovis på {{day}}',
  'Monthly on day {{day}}': 'Månadsvis på dag {{day}}',
  'Yearly on {{day}} {{month}}': 'Årligen på {{day}} {{month}}',
  Mon: 'mån',
  Tue: 'Tis',
  Wed: 'ons',
  Thu: 'tors',
  Fri: 'fre',
  Sat: 'lör',
  Sun: 'Solen',
  January: 'januari',
  February: 'februari',
  March: 'Mars',
  April: 'april',
  May: 'Maj',
  June: 'Juni',
  July: 'Juli',
  August: 'Augusti',
  September: 'september',
  October: 'oktober',
  November: 'november',
  December: 'December',
  'Day(s)': 'Dag(ar)',
  'Week(s)': 'Vecka(or)',
  'Month(s)': 'Månad(er)',
  'Year(s)': 'År/år',
  'Show details': 'Visa detaljer',
  'Hide details': 'Dölj detaljer',
  Loading: 'Belastning',
  'No fields defined': 'Inga fält definierade',
  Copy: 'Kopiera',
  'Copy of': 'Kopia av',
  eForm: 'e-formulär',
  'Report headline': 'Rapportrubrik',
  Title: 'Titel',
  'Set tags...': 'Ställ in taggar...',
  'Assign to...': 'Tilldela till...',
  'Add description': 'Lägg till beskrivning',
  'My properties': 'Mina fastigheter',
  'My teams': 'Mina lag',
  'My employees': 'Mina anställda',
  'Create employee': 'Skapa anställd',
  day: 'dag',
  week: 'vecka',
  month: 'månad',
  year: 'år',
  'Custom repeat': 'Anpassad upprepning',
  'Repeat on': 'Upprepa på',
  Ends: 'Slutar',
  Never: 'Aldrig',
  On: 'På',
  After: 'Efter',
  occurrences: 'händelser',
  Done: 'Gjort',
  'Could not save the event': 'Kunde inte spara händelsen',
  Error: 'Fel',
  'At least one worker must be assigned': 'Minst en arbetare måste tilldelas',
  'Previous week': 'Föregående vecka',
  'Next week': 'Nästa vecka',
  'Previous day': 'Föregående dag',
  'Next day': 'Nästa dag',
  'Could not create report headline': 'Kunde inte skapa rapportrubrik',
  'Create new': 'Skapa nytt',
  'Change property': 'Ändra egenskap',
  Wk: 'Vecka',
  'Weekly every {{days}}': 'Veckovis var {{days}}',
  'Weekly on all days': 'Veckovis alla dagar',
  'Every {{n}} weeks: {{days}}': 'Var {{n}} vecka: {{days}}',
  'Every {{n}} weeks: all days': 'Var {{n}} vecka: alla dagar',
  'Every {{n}} days': 'Var {{n}} dag',
  'Every {{n}} months on day {{dom}}': 'Var {{n}} månad på dag {{dom}}',
  'Every {{n}} years on {{dom}}. {{month}}': 'Var {{n}} år på {{dom}}. {{month}}',
  'Attach file': 'Bifoga fil',
  'Save event first to attach files': 'Spara händelsen först för att bifoga filer',
  'Delete attachment?': 'Ta bort bilagan?',
  'Could not download file': 'Kunde inte ladda ner filen',
  'Pending save': 'Väntar på att spara',
  'Add Google Drive file': 'Lägg till Google Drive-fil',
  'Loading Google Picker': 'Läser in Google Picker',
  'Connect Google Drive': 'Anslut Google Drive',
  'Drive disconnected': 'Enheten frånkopplad',
  'Reconnect to resume sync': 'Återanslut för att återuppta synkroniseringen',
  'Synced from Google Drive': 'Synkroniserad från Google Drive',
  'Could not connect to Google Drive': 'Kunde inte ansluta till Google Drive',
  'Could not attach Drive file': 'Kunde inte bifoga Drive-filen',
  'Google Drive connected': 'Google Drive ansluten',
  'You can close this window': 'Du kan stänga det här fönstret',
  'Open in Google Drive': 'Öppna i Google Drive',
  'Connected Google Drive accounts': 'Kopplade Google Drive-konton',
  'No Google Drive accounts connected.': 'Inga Google Drive-konton är anslutna.',
  Disconnect: 'Koppla från',
  'Disconnect Google Drive?': 'Koppla bort Google Drive?',
  'Disconnecting will stop sync for any Drive-attached files.': 'Om du kopplar bort anslutningen stoppas synkroniseringen för alla Drive-bifogade filer.',
  'Connected on': 'Ansluten den',
  'Last used': 'Senast använd',
  'Drive disconnected — reconnect to resume sync': 'Enheten frånkopplad — återanslut för att återuppta synkroniseringen',
  'Last refreshed': 'Senast uppdaterad',
  'Google Drive disconnected': 'Google Drive frånkopplad',
  'Google Drive disconnected — reconnect to resume sync': 'Google Drive frånkopplad — återanslut för att återuppta synkroniseringen',
  'All weekdays (Monday to Friday)': 'Alla vardagar (måndag till fredag)',
  'Monthly on the {{ordinal}} {{day}}': 'Månadsvis på den {{ordinal}} {{day}}',
  'Every {{n}} months on the {{ordinal}} {{day}}': 'Var {{n}} månad på den {{ordinal}} {{day}}',
  'Overdue task shown in app': 'Försenad uppgift visas i appen',
  'Overdue task not shown in app': 'Försenad uppgift visas inte i appen',
  'Repeat sub-type': 'Upprepa undertyp',
  'Monthly day': 'Månadsdag',
  'Monthly on the first': 'Månadsvis den första',
  Translate: 'Översätta',
  Farve: 'Färg',
  'Tasks and actions': 'Uppgifter och åtgärder',
  Compliance: 'Efterlevnad',
  'Export CSV': 'Exportera CSV',
  Search: 'Söka',
  Cancel: 'Avboka',
  Create: 'Skapa',
  'Team name': 'Lagnamn',
  'Delete team': 'Ta bort laget',
  'Create team': 'Skapa team',
  'Tag name': 'Taggnamn',
  'Create tag': 'Skapa tagg',
  'Enter tag name': 'Ange taggnamn',
  'Enter team name': 'Ange lagets namn',
  Confirm: 'Bekräfta',
  'Save changes': 'Spara ändringar',
  'Complete task': 'Slutför uppgiften',
  'Only this': 'Endast detta',
  'This and following': 'Detta och följande',
  'All in series': 'Alla i serie',
  'Date is required': 'Datum krävs',
  'Open link': 'Öppna länken',
  'No worker found': 'Ingen arbetare hittades',
  'My tags': 'Mina taggar',
  ID: 'ID',
  Save: 'Spara',
  Delete: 'Radera',
  Edit: 'Redigera',
  'Select date': 'Välj datum',
  'Edit repeat': 'Redigera upprepning',
  'Move repeat': 'Flytta upprepa',
  'Delete repeat': 'Ta bort upprepning',
  'At least one worker or worker tag must be assigned': 'Minst en arbetare eller arbetartagg måste tilldelas',
  'Assign to worker tags': 'Tilldela till arbetartaggar',
  'This and following (incl. completed)': 'Detta och följande (inkl. färdigställda)',
  'Payroll rules': 'Löneregler',
  'Pay Rule Set': 'Löneregeluppsättning',
  'Select the pay rule set for this worker': 'Välj löneregeluppsättningen för den här arbetaren',
  'View pay rule set': 'Visa uppsättning löneregler',
  None: 'Ingen',
  'Advanced settings': 'Avancerade inställningar',
  'Use 1-minute intervals': 'Använd 1-minutsintervaller',
  'Enable 1-minute granularity for time entry. Default increments are 5 or 15 minutes.': 'Aktivera 1-minuts granularitet för tidsinmatning. Standardintervallen är 5 eller 15 minuter.',
  'Once enabled, 1-minute intervals cannot be turned off': 'När 1-minutsintervaller är aktiverade kan de inte stängas av',
  OK: 'OK',
  'My calendars': 'Mina kalendrar',
  'Task visible on calendar and shown in app': 'Uppgift synlig i kalendern och visas i appen',
  'Task dimmed on calendar and not shown in app': 'Uppgift nedtonad i kalendern och visas inte i appen',
  Calendar: 'Kalender',
  'Create calendar': 'Skapa kalender',
  'Calendar name': 'Kalendernamn',
  'Delete calendar': 'Ta bort kalender',
  'This will permanently delete the calendar and its {{count}} events. This cannot be undone.': 'Detta kommer att permanent radera kalendern och dess {{count}} händelser. Detta kan inte ångras.',
  'Enter calendar name': 'Ange kalendernamn',
  'Are you sure you want to delete the calendar': 'Är du säker på att du vill ta bort kalendern',
  'All tasks associated with this calendar will lose their calendar association': 'Alla uppgifter som är kopplade till den här kalendern kommer att förlora sin kalenderkoppling',
  'All properties': 'Alla fastigheter',
  'All calendars': 'Alla kalendrar',
  'All tags': 'Alla taggar',
  'All employees': 'Alla anställda',
  'Not completed tasks': 'Ej slutförda uppgifter',
  'Completed tasks': 'Slutförda uppgifter',
  'All tasks': 'Alla uppgifter',
  '1 month': '1 månad',
  '3 months': '3 månader',
  '6 months': '6 månader',
  '12 months': '12 månader',
  'Year to date': 'Hittills i år',
  'Set period': 'Ställ in period',
  'Show report': 'Visa rapport',
  'Download as': 'Ladda ner som',
  Download: 'Ladda ner',
  'Show all': 'Visa alla',
  Showing: 'Visande',
  'Select filters and click Show report': 'Välj filter och klicka på Visa rapport',
  'No tasks in the selected period': 'Inga uppgifter i den valda perioden',
  'Delete log': 'Ta bort logg',
  'This action cannot be undone': 'Den här åtgärden kan inte ångras',
  Employees: 'Anställda',
  At: 'På',
  'Time of day': 'Tid på dagen',
  'Done date': 'Klardatum',
  'Completed by': 'Slutförd av',
  View: 'Se',
  'Task visible on calendar': 'Uppgift synlig i kalendern',
  'Task dimmed on calendar': 'Uppgift nedtonad i kalendern',
  'This task is part of a recurring series. What do you want to edit?': 'Den här uppgiften är en del av en återkommande serie. Vad vill du redigera?',
  'This task is part of a recurring series. What do you want to move?': 'Den här uppgiften är en del av en återkommande serie. Vad vill du flytta?',
  'This task is part of a recurring series. What do you want to delete?': 'Den här uppgiften är en del av en återkommande serie. Vad vill du ta bort?',
  'Updated.': 'Uppdaterad.',
  'Select / create report headline': 'Välj/skapa rapportrubrik',
  'No tasks this month': 'Inga uppgifter denna månad',
  'Previous month': 'Föregående månad',
  'Next month': 'Nästa månad',
  'No tasks this week': 'Inga uppgifter denna vecka',
  Id: 'Id',
  Action: 'Handling',
  Employee: 'Anställd',
  'tasks selected': 'valda uppgifter',
  'Show paginated': 'Visa paginerad',
  'From employee': 'Från anställd',
  'To employee': 'Till anställd',
  'Move selected to employee': 'Flytta markerade till medarbetare',
  'Move from employee to employee': 'Gå från anställd till anställd',
  'Add employee': 'Lägg till anställd',
  'Change eForm': 'Ändra e-formulär',
  'The eForm will be changed immediately for the selected tasks': 'E-formuläret kommer att ändras omedelbart för de valda uppgifterna',
  'Add tags': 'Lägg till taggar',
  'Remove tags': 'Ta bort taggar',
  'Set compliance': 'Ställ in efterlevnad',
  'Copy to property': 'Kopiera till egendom',
  'Delete selected': 'Ta bort markerade',
  'Collected data will not be deleted': 'Insamlad data kommer inte att raderas',
  Tasks: 'Uppgifter',
  'Batch action': 'Batchåtgärd',
  'Select batch action': 'Välj batchåtgärd',
  'Delete photo': 'Radera foto',
  'Delete photo?': 'Radera foto?',
  'The photo will be removed from the task. This action cannot be undone.':
    'Fotot tas bort från uppgiften. Åtgärden kan inte ångras.',
  'The eForm applies to the whole series. Changing it updates every occurrence that has not been completed yet, including occurrences in the past. Completed occurrences keep the eForm they were completed with.': 'E-formuläret gäller för hela serien. Om du ändrar det uppdateras alla förekomster som ännu inte har slutförts, inklusive förekomster som gjorts tidigare. Slutförda förekomster behåller det e-formulär de fylldes i med.',
  'Select a calendar': 'Välj en kalender',
  'Assigned workers': 'Tilldelade medarbetare',
  'Other workers': 'Övriga medarbetare',
  Sections: 'Avsnitt',
  // Task list batch: change start date (#1122). The preview panel's four
  // counts are one key each so each language can word-order them freely.
  'Change start date': 'Ändra startdatum',
  '{{count}} tasks': '{{count}} uppgifter',
  '{{count}} open occurrences will be retracted': '{{count}} öppna förekomster återkallas',
  '{{count}} completed will be preserved': '{{count}} slutförda bevaras',
  '{{count}} overdue tasks will be created': '{{count}} försenade uppgifter skapas',
  'Calculating preview': 'Beräknar förhandsgranskning',
  'Preview could not be loaded': 'Förhandsgranskningen kunde inte laddas',
  'Pick a start date to preview the change': 'Välj ett startdatum för att förhandsgranska ändringen',
  // Task list batch: activate / deactivate (#1123). The two radio labels
  // reuse the existing calendar-modal status keys; only the action label
  // and the honest deactivate warning are new.
  'Activate / deactivate': 'Aktivera / inaktivera',
  'Deactivating retracts the open occurrences from the app. Completed occurrences and the data already collected are preserved.': 'Inaktivering drar tillbaka de öppna förekomsterna från appen. Slutförda förekomster och de data som redan samlats in bevaras.',
  'Update table': 'Uppdatera tabell',
  'Select filters and click Update table': 'Välj filter och klicka på Uppdatera tabell',
  'Compliance overview': 'Översikt',
  'Compliance details': 'Detaljer',
  'Compliance report': 'Rapportera',
  'Overview shows both completed and not completed tasks': 'Översikten visar både slutförda och ej slutförda uppgifter',
  'Select a property to filter by calendar': 'Välj en egendom att filtrera efter kalender',
  'Select both a start and an end date': 'Välj både ett start- och ett slutdatum',
  'The end date must be on or after the start date': 'Slutdatumet måste vara samma som eller efter startdatumet',
  'Showing all': 'Visar alla',
  'No results': 'Inga resultat',
  Filtering: 'Filtrering',
  'View mode': 'Visningsläge',
  Pagination: 'Paginering',
  'No tasks match the selected filters.': 'Inga uppgifter matchar de valda filtren.',
  'Task done': 'Uppgift klar',
  'Task not done': 'Uppgiften är inte klar',
  'Could not load the data': 'Kunde inte ladda data',
  // Backfill of keys present in enUS.ts but missing here (#1167).
  // Machine-translated via translateTsFiles.py; review welcome.
  'Adhoc overview': 'Ad hoc-översikt',
  Overview: 'Översikt',
  History: 'Historia',
  'New task': 'Ny uppgift',
  Content: 'Innehåll',
  'Last deadline': 'Sista deadline',
  Overdue: 'Försenad',
  Solved: 'Löst',
  Archived: 'Arkiverad',
  'With comments': 'Med kommentarer',
  'Without comments': 'Utan kommentarer',
  'Choose who completes the task': 'Välj vem som slutför uppgiften',
  'Task can be seen and completed by': 'Uppgiften kan ses och slutföras av',
  'Assigned only': 'Endast tilldelad',
  Everyone: 'Alla',
  'Execute no later than and first display': 'Kör senast och första visningen',
  'Last 30 days': 'Senaste 30 dagarna',
  'Last 60 days': 'Senaste 60 dagarna',
  'Last 90 days': 'Senaste 90 dagarna',
  'Last 6 months': 'Senaste 6 månaderna',
  'Last 12 months': 'Senaste 12 månaderna',
  'Last 24 months': 'Senaste 24 månaderna',
  'Custom period': 'Anpassad period',
  Columns: 'Kolumner',
  Created: 'Skapad',
  'No deadline': 'Ingen deadline',
  'Deadline today': 'Sista dag idag',
  '{{count}} days until deadline': '{{count}} dagar kvar till deadline',
  And: 'OCH',
  Or: 'ELLER',
  Weekdays: 'Vardagar',
  'Deadline reminder': 'Påminnelse om deadline',
  'First visibility reminder': 'Första påminnelsen om synlighet',
  Comments: 'Kommentarer',
  Period: 'Period',
  Filters: 'Filter',
  From: 'Från',
  To: 'Till',
  'Archive task': 'Arkivuppgift',
  created: 'Skapad',
  assigned: 'Tilldelad',
  completed: 'Avslutad',
  archived: 'Arkiverad',
  commented: 'Kommenterade',
  '30 days': '30 dagar',
  '60 days': '60 dagar',
  '90 days': '90 dagar',
  '6 months period': '6 månader',
  '12 months period': '12 månader',
  'Period and filter': 'Period och filter',
  'All areas': 'Alla områden',
  'Select a property to filter by area': 'Välj en fastighet att filtrera efter område',
  'Tags in history (AND)': 'Taggar i historiken (OCH)',
  'Search tags': 'Sök efter taggar …',
  'No tags match the search': 'Inga taggar matchar sökningen.',
  'No tags yet - create them under Overview': 'Inga taggar ännu — skapa dem under Översikt.',
  'Remove tag {{name}}': 'Ta bort «{{name}}»',
  'None selected': 'Ingen vald',
  'Active period': 'Aktiv period',
  '{{from}} to {{to}} inclusive of both days': '{{from}} — {{to}} (inklusive båda dagarna).',
  'History completed tasks': 'Historik — slutförda uppgifter',
  'Completed date': 'Avslutad',
  'Task resolved status': 'Löst',
  'No tasks match the selected period, property, area and tags': 'Inga uppgifter matchar vald period, egenskap, område och taggar.',
  'Previous page': 'Tidigare',
  'Next page': 'Nästa',
  'Page {{page}} of {{pages}}': 'Sida {{page}} av {{pages}}',
  hit: 'hit',
  hits: 'träffar',
  'Show help text': 'Visa hjälptext',
  'Hide help text': 'Dölj hjälptext',
  'Completion date term': 'Slutförandedatum',
  'is the date of': 'är datumet för',
  'when the task was completed': '(när uppgiften var slutförd).',
  is: 'är',
  'inclusive of both ends': 'inklusive båda ändar',
  'whole days by calendar date': '(hela dagar efter kalenderdatum).',
  'Fixed periods are counted backwards from': 'Fasta perioder räknas baklänges från',
  'Day presets term': '30 / 60 / 90 dagar',
  'equal the stated number of calendar days including today': '= det angivna antalet kalenderdagar inklusive idag.',
  and: 'och',
  'equal the same calendar day shifted back': '= samma kalenderdag flyttad tillbaka det antalet månader (justerad om dagen inte finns i den månaden).',
  'below the active period line narrow the list to': 'under raden «Aktiv period:» begränsar du listan till den valda egenskapen och — när en egenskap har valts — till ett specifikt område eller till',
  'all areas term': 'alla områden',
  'on that property': 'på den fastigheten.',
  'The area dropdown is only active once a specific property is selected': 'Listrutan för område är bara aktiv när du har valt en specifik fastighet (inte «Alla fastigheter»).',
  'Tags AND term': 'Taggar (OCH)',
  'when one or more tags are selected only tasks that have': ': när en eller flera taggar är valda, endast uppgifter som har',
  'all term': 'alla',
  'selected tags are shown without selected tags every completed task in the period is included': 'Valda taggar visas. Utan valda taggar inkluderas alla slutförda uppgifter under perioden.',
  'can be hidden or shown with the header line': 'kan döljas eller visas med rubrikraden (standard: stängd). Gruppen samlas',
  Add: 'Tillägga',
  'Area name is empty or already exists': 'Områdesnamnet är tomt eller finns redan',
  'Create areas': 'Skapa områden',
  'Create one or more areas on the selected property (one name per line)': 'Skapa ett eller flera områden på den valda fastigheten (ett namn per rad)',
  'Delete area': 'Ta bort område',
  'Edit or delete areas for the selected property': 'Redigera eller ta bort områden för den valda egendomen',
  'Existing tasks keep their history': 'Befintliga uppgifter behåller sin historik',
  'Failed to create areas': 'Misslyckades med att skapa områden',
  'Failed to delete area': 'Misslyckades med att ta bort området',
  'Manage areas': 'Hantera områden',
  'No areas on this property yet': 'Inga områden på den här fastigheten ännu',
  'One area name per line': 'Ett områdesnamn per rad',
  'Rename area': 'Byt namn på område',
  'Compliance %': 'Efterlevnad %',
  'Compliance total': 'Total',
  'No properties match the selected filters.': 'Inga egenskaper matchar de valda filtren.',
  'Sort by': 'Sortera efter',
  'Completion date': 'Slutförandedatum',
  'Without tag': 'Utan etikett',
  'Columns unavailable': 'Kolumner är inte tillgängliga',
  '1 image': '1 bild',
  '{{count}} images': '{{count}} bilder',
  'Only completed tasks can be edited': 'Endast slutförda uppgifter kan redigeras',
};
