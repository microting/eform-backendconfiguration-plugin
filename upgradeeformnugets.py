...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("eFormAPI/Plugins/BackendConfiguration.Pn/BackendConfiguration.Pn")
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microting.EformBackendConfigurationBase',
            'Microting.ItemsPlanningBase', 'Microting.TimePlanningBase',
            'Microting.EformAngularFrontendBase', 'Microting.eFormCaseTemplateBase',
            'Google.Apis.Sheets.v4', 'Sentry'
        ]
        process_repository('BackendConfiguration.Pn.csproj', packages, 'eform-angular-backendconfiguration-plugin')

        os.chdir("..")
        os.chdir("BackendConfiguration.Pn.Integration.Test")
        packages = [
            'Microsoft.NET.Test.Sdk', 'NSubstitute', 'NUnit', 'NUnit3TestAdapter',
            'NUnit.Analyzers', 'Testcontainers', 'Testcontainers.Mariadb'
        ]
        process_repository('BackendConfiguration.Pn.Integration.Test.csproj', packages, 'eform-angular-backendconfiguration-plugin')

        os.chdir("..")
        os.chdir("BackendConfiguration.Pn.Test")
        packages = [
            'Microsoft.NET.Test.Sdk', 'NSubstitute', 'NUnit', 'NUnit3TestAdapter',
            'NUnit.Analyzers', 'Testcontainers', 'Testcontainers.Mariadb'
        ]
        process_repository('BackendConfiguration.Pn.Test.csproj', packages, 'eform-angular-backendconfiguration-plugin')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            run_command(f"github_changelog_generator -u microting -p eform-angular-backendconfiguration-plugin -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
