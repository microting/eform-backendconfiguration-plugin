...
[TestFixture]
public class BackendConfigurationAreaRulePlanningsServiceHelperTestLogBooksCustomStartDate : TestBaseSetup
{
    // Opted out of the fixture-scoped schema replay:
    // same ~410-line clone shape as LogBooks.
...
Assert.That(folderTranslations[31].Name, Is.EqualTo("00. Завдання, що прострочені"));
        ...
        var diff = (now - new DateTime(now.Year, 4, 12)).TotalDays;
...
Assert.That(folderTranslations[31].Name, Is.EqualTo("00. Завдання, що прострочені"));
    ...
    public async Task UpdatePlanning_AreaRuleDays4DisableReenable_ReturnsSuccess()
    {
        ...
        var nextExecutionTime =
            new DateTime(2023, 4, 12).AddDays(multiplier * 4);
        if (nextExecutionTime < now)
        {
