...
[TestFixture]
public class BackendConfigurationAreaRulePlanningsServiceHelperSlurryTanksFillData : TestBaseSetup
{
    // Should test the UpdatePlanning method for area rule "03. Gyllebeholdere" for areaRule: 0 with construction only enabled
    // Fill data and add another worker to the list of assigned sites
    [Test]
    ...
    // Should test the UpdatePlanning method for area rule "03. Gyllebeholdere" for areaRule: 0 with alarm and construction
    // Fill data for both cases and add another worker to the list of assigned sites
    [Test]
    public async Task UpdatePlanning_AreaRule0AlamAndConstructionFillBoth_ReturnsSuccess()
    {
        // Arrange
        var core = await GetCore();
        var propertyCreateModel = new PropertyCreateModel
        ...
        var itemPlanningCase = await ItemsPlanningPnDbContext!.PlanningCases.ToListAsync();
